...
----------------------------
A Pygame application that displays a face capable of expressing
various emotions. The face has a neutral state with random eye
movements and can transition to happy, angry, sad, and pamper
emotions via keyboard shortcuts.
...
def generate_smile_data(progress, y_offset=0):
    """Generates the polygon points for a dynamic smile."""
...
def generate_angry_mouth_points(progress, offset=(0,0)):
    """Generates the points for a jagged angry mouth line."""
    if progress <= 0: return None
...
def draw_lids(surface, x, y, width, height, blink_progress=0, squint_progress=0):
    """Draws eyelids for blinking and squinting."""
    if blink_progress > 0:
...
def draw_tear(surface, x, y, size, progress, offset=(0,0)):
    """Draws an inverted, glistening tear."""
    cfg = SAD_CONFIG
...
def draw_blush(surface, x, y, radius_x, radius_y):
    """Draws an elliptical blush mark."""
...
def draw_neutral_emotion():
    """Draws the default neutral face with blinking and eye movement."""
    ...
    for i, style in enumerate([left_style, right_style]):
        ...
        if style == 'normal':
            draw_rounded_eye(screen, base_x, EYE_Y_POS, EYE_WIDTH, EYE_HEIGHT, EYE_CORNER_RADIUS)
            draw_lids(screen, base_x, EYE_Y_POS, EYE_WIDTH, EYE_HEIGHT, lid_progress)
        elif style == 'enlarged':
            ew, eh = int(EYE_WIDTH * 1.3), int(EYE_HEIGHT * 1.3)
            ex, ey = base_x - (ew - EYE_WIDTH) // 2, EYE_Y_POS - (eh - EYE_HEIGHT) // 2
            draw_rounded_eye(screen, ex, ey, ew, eh, EYE_CORNER_RADIUS)
            draw_lids(screen, ex, ey, ew, eh, lid_progress)
        elif style == 'circle':
            d = min(EYE_WIDTH, EYE_HEIGHT)
            ex, ey = base_x + (EYE_WIDTH - d) // 2, EYE_Y_POS + (EYE_HEIGHT - d) // 2
...
def draw_happy_emotion(current_time, start_time, progress):
    ...
    for dx in [-EYE_SPACING, EYE_SPACING]:
        ...
        draw_rounded_eye(screen, new_x, new_y + bounce_offset, new_width, new_height, EYE_CORNER_RADIUS)
...
while running:
    ...
    if current_emotion != "neutral":
        # An emotion is active, handle its animation progress
        ...
        # Revert to neutral after hold time
        if now - emotion_start_time > (EMOTION_ANIM_DURATION + EMOTION_HOLD_DURATION):
            current_emotion = "neutral"
