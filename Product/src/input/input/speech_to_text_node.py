...
# --- KEY CHANGE: We will now find the microphone by a unique part of its name ---
TARGET_MICROPHONE_KEYWORD = "Audio Array" # This should be unique to your desired microphone
...
# --- Helper function to find the microphone index by name ---
def find_microphone_index(keyword):
    """
    Finds the index of a microphone that contains the given keyword in its name.
    """
...
class SpeechToTextNode(Node):
    ...
    /robot_state topic to prevent feedback loops. Now auto-detects the microphone.
    """
    def __init__(self):
        ...
        # --- KEY CHANGE: Auto-detect microphone ---
        self.get_logger().info(f"Searching for microphone containing '{TARGET_MICROPHONE_KEYWORD}'...")
        mic_index = find_microphone_index(TARGET_MICROPHONE_KEYWORD)

        if mic_index is None:
            self.get_logger().error(f"❌ CRITICAL: Could not find microphone with name containing '{TARGET_MICROPHONE_KEYWORD}'.")
            self.get_logger().error("Please check the microphone connection and the TARGET_MICROPHONE_KEYWORD constant.")
            # Print available mics to help with debugging
            self.get_logger().info("Available microphones are:")
            for i, name in enumerate(sr.Microphone.list_microphone_names()):
                self.get_logger().info(f'  Index {i}: "{name}"')
            rclpy.shutdown()
            return

        self.get_logger().info(f"✅ Success! Found '{TARGET_MICROPHONE_KEYWORD}' microphone at index {mic_index}.")
        try:
            self.microphone = sr.Microphone(device_index=mic_index)
        except Exception as e:
            self.get_logger().error(f"Error initializing microphone at index {mic_index}: {e}")
            rclpy.shutdown()
            return
...
def main(args=None):
    ...
    # Check if rclpy was shut down due to a mic error
    if rclpy.ok():
        speech_to_text_node = SpeechToTextNode()
        if rclpy.ok(): # Check again in case mic init failed
