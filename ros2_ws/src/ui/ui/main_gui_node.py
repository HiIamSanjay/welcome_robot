...
# --- UI Configuration (from your reference script) ---
...
class MainGuiNode(Node):
    ...
    def set_robot_state(self, state):
        """Publishes the current state of the robot (e.g., 'speaking', 'listening')."""
        msg = String()
        msg.data = state
    ...
    def setup_ui(self):
        """Initializes all the tkinter widgets for the GUI."""
    ...
    # --- KEY CHANGE: Function to interrupt speech ---
    def interrupt_speech(self):
        """Stops any ongoing TTS playback."""
        if pygame.mixer.music.get_busy():
            pygame.mixer.music.stop()
            self.get_logger().info("TTS playback interrupted by user.")
            # The finally block in speak_text will handle the state change

    def on_text_input_pressed(self):
        """Handles the text input button press."""
    ...
    def speak_text(self, text):
        """Generates and plays speech, managing the robot's state."""
        ...
        try:
            ...
            self.set_robot_state("listening")
            self.get_logger().info("Finished speaking (or was interrupted).")
    ...
    def user_text_callback(self, msg):
        self.interrupt_speech() # Interrupt speech if user starts talking
