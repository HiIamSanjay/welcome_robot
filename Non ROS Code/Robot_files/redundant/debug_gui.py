...
# EMOTION_CONFIRMATION_DURATION = 0.1 # DEBUG: Removed as per request
...
class AIAssistantApp:
    ...
    def __init__(self, root):
        ...
        # --- DEBUG: Log available microphones at startup ---
        self._log_microphone_info()
    ...
    def _log_microphone_info(self):
        """Logs available microphone names for debugging."""
        self._log("🎤 DEBUG: Listing available microphones...")
        try:
            mic_names = sr.Microphone.list_microphone_names()
            if not mic_names:
                self._log("🎤 DEBUG: No microphones found by SpeechRecognition library.")
                return
            for i, name in enumerate(mic_names):
                self._log(f"  - Mic Index {i}: {name}")
        except Exception as e:
            self._log(f"🎤 DEBUG: Could not list microphones. Error: {e}")
    ...
    def update_video_frame(self):
        # --- PI CAMERA FIX: Prioritize virtual camera, then auto-detect ---
        if not self.cap or not self.cap.isOpened():
            # First, try the specific virtual camera index from your script
            self._log("📹 Trying to open virtual camera at index 2...")
            cap = cv2.VideoCapture(2)
            if cap.isOpened():
                self.cap = cap
                self._log("✅ Virtual camera found and opened on index 2.")
            else:
                self._log("⚠️ Virtual camera at index 2 not found. Searching for others...")
                # Fallback to searching other cameras if the virtual one isn't available
                for i in range(5, -1, -1):
                    if i == 2: continue # Skip index 2 as we've already tried it
                    self._log(f"📹 Trying to open camera index {i}...")
                    cap = cv2.VideoCapture(i)
                    if cap.isOpened():
                        self.cap = cap
                        self._log(f"✅ Camera found and opened on index {i}.")
                        break
            if not self.cap:
                self._log("❌ Could not find any working camera.")
                return

        ret, frame = self.cap.read()
        if not ret:
            # Add a small delay and try to reconnect if the frame read fails
            time.sleep(0.1)
            return
        ...
        # --- EMOTION LOGIC (Confirmation timer removed) ---
        if self.state == AppState.MIRRORING and face_currently_present:
            try:
                ...
                # Instantly update if the emotion has changed
                if detected_emotion != self.confirmed_emotion:
                    self._log(
                        f"🙂 User emotion mirrored to: {detected_emotion}")
                    self.confirmed_emotion = detected_emotion
                    self._send_eye_command(self.confirmed_emotion.lower())

            except Exception as e:
                self._log(f"⚠️ DeepFace analysis failed during mirroring: {e}")
    ...
    def background_voice_listener(self):
        ...
        while True:
            # Check if we are in a state that requires listening
            if self.state in [AppState.FACE_DETECTED, AppState.LISTENING_FOR_FOLLOW_UP, AppState.MIRRORING]:
                self._log("🎤 DEBUG: Voice listener is in an active state.")
                try:
                    self._log("🎤 DEBUG: Attempting to open microphone...")
                    with sr.Microphone() as source:
                        self._log("🎤 DEBUG: Microphone opened successfully.")
                        self._log("🎤 DEBUG: Calibrating for ambient noise (0.5s)...")
                        self.recognizer.adjust_for_ambient_noise(source, duration=0.5)
                        self._log(f"🎤 DEBUG: Ambient noise calibrated. Energy threshold: {self.recognizer.energy_threshold:.2f}")
                        self._log(f"🎤 DEBUG: Now attempting to listen for up to {PHRASE_TIME_LIMIT} seconds...")
                        
                        audio = self.recognizer.listen(
                            source, phrase_time_limit=PHRASE_TIME_LIMIT)
                        self._log("🎤 DEBUG: `listen()` completed. Audio captured.")

                    self._log("🎤 DEBUG: Attempting to recognize speech via Google...")
                    text = self.recognizer.recognize_google(audio)
                    self._log("🎤 DEBUG: Recognition successful.")
                    self._log(f"🎤 You said: '{text}'")
                    self.last_query = text
                    # Use root.after to safely interact with the GUI from this thread
                    self.root.after(0, self.process_ai_query)
                
                except sr.UnknownValueError:
                    self._log("🎤 DEBUG: Google Speech Recognition could not understand audio.")
                except sr.RequestError as e:
                    self._log(f"🎤 DEBUG: Could not request results from Google service; {e}")
                except Exception as e:
                    self._log(f"🎤 DEBUG: An unexpected error occurred in the voice listener: {e}")
