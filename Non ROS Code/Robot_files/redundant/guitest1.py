...
Analyze the user's query and generate a helpful response. After generating the response, you MUST classify its emotional tone.
On a new, separate line, add ONE of the following keywords: HAPPY, SAD, ANGRY, PAMPER, or NEUTRAL.
"""
    ...
    def background_voice_listener(self):
        ...
        while True:
            if self.state in [AppState.FACE_DETECTED, AppState.LISTENING_FOR_FOLLOW_UP, AppState.MIRRORING]:
                try:
                    # --- MIC FIX: Create a fresh Recognizer instance for each listening attempt ---
