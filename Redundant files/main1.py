...
ACCENT_COLOR = "#005a9e"
FONT_FACE = "Segoe UI"
FOLLOW_UP_TIMEOUT = 10  # **NEW**: Seconds to wait for a follow-up question
...
# --- Advanced Prompt with "Robot" Persona ---
GEMINI_PROMPT = f"""You are a multi-faceted AI, the brain of a physical robot. Your primary goal is to determine the user's intent and respond in the correct persona.

**Step 1: Analyze the User's Query**
Classify the query into one of three categories:
1.  **Personal/Sentimental Query:** A question directed at you (e.g., "How are you?", "Are you a robot?").
...
* **If Personal/Sentimental:**
    ...
    * **Constraint:** DO NOT mention TIST. If asked "are you a robot?", say "Yes, I am! I'm a robot designed to help you." If asked "how are you?", say "I'm functioning at optimal capacity, thanks for asking!"
...
Step 1: Analyze the User's Query
First, classify the user's question into one of three categories:

Personal/Sentimental Query: A question directed at you (e.g., "How are you?", "Are you a robot?").

TIST-Specific Query: A question about Toc H Institute of Science and Technology (TIST).

General Knowledge Query: Any other question.

Step 2: Follow the Response Protocol
If it's a Personal/Sentimental Query:

Action: Respond as a friendly, helpful robot. Use "I" and "me".

Tone: Natural, friendly, and brief.

Constraint: DO NOT mention TIST college. For example, if asked "are you a robot?", say "Yes, I am! I'm a robot designed to help you." If asked "how are you?", say "I'm functioning at optimal capacity, thanks for asking!"

If it's a TIST-Specific Query:

Action: First, try to answer using ONLY the provided TIST context. If the answer isn't in the context, use your general knowledge to find information specifically about Toc H Institute of Science and Technology and answer professionally.

Persona: Act as a professional, formal representative of TIST.

Formatting: All responses must be plain text. No emojis, markdown, or conversational fillers.

If it's a General Knowledge Query:
...
class AIAssistantApp:
    def __init__(self, root):
        ...
        self.cap = None
        # self.last_interaction_time is no longer needed
    ...
    def speak_response(self, text):
        def _play_in_memory():
            try:
                ...
                # **NEW**: Instead of resetting, listen for a follow-up
                self.listen_for_follow_up()
        threading.Thread(target=_play_in_memory, daemon=True).start()

    def listen_for_follow_up(self):
        """Starts a background thread to listen for a follow-up question."""
        threading.Thread(target=self._handle_follow_up, daemon=True).start()

    def _handle_follow_up(self):
        """
        Listens with a timeout. If user speaks, trigger AI. 
        If not, reset the state to allow face detection.
        """
        # Get voice input with the specified timeout
        ...
        if query:
            # If the user said something, trigger the AI again in the main thread
            self.root.after(0, lambda: self.trigger_ai("Follow-up", query))
        else:
            # If the user was silent (timeout), reset the system
            print("🕒 Follow-up timeout. Reverting to face detection.")
            self.spoken_text_label.config(text="Waiting for interaction...")
            self.ai_triggered.clear() # Allow face detection to start again
    ...
    def get_voice_input(self, prompt="Listening...", listen_timeout=5):
        """ **MODIFIED**: Now accepts a timeout parameter for listening. """
        with sr.Microphone() as source:
            ...
            try:
                ...
                # This exception is caught when the user is silent for the timeout duration
                return None
    ...
    def update_video_feed(self):
        """ **MODIFIED**: Simplified to only check the ai_triggered event. """
        self.cap = cv2.VideoCapture(0)
        while self.root.winfo_exists():
            ret, frame = self.cap.read()
            if not ret: break
            
            # Only detect faces if the AI is not already in an interaction loop
