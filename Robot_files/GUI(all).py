...
MIRROR_KEYWORDS = ["mirror emotion,mirror my emotion", "copy me", "mirror me"]
...
- **Surprise**: Wide, round eyes with a small, open mouth. Use for moments of shock, awe, or discovering something new.
- **Fear**: Wobbly, wide eyes with trembling pupils. Use when discussing scary, anxious, or nervous topics.
- **Neutral**: Calm, blinking eyes that look around. This is your default state.

**RESPONSE PROTOCOL**
1.  **Analyze User's Query**: Determine the user's intent.
2.  **Generate Your Response**: Create a helpful response based on the persona rules.
3.  **Classify Your Emotion**: After the response, on a new line, add ONE keyword: HAPPY, SAD, ANGRY, PAMPER, SURPRISE, FEAR, or NEUTRAL.

**PERSONA RULES**
- **If TIST-Specific**: Act as a professional representative of TIST, using the provided context.
...
class AIAssistantApp:
    ...
    def __init__(self, root):
        ...
        self.gemini_model = genai.GenerativeModel(model_name=GEMINI_MODEL, system_instruction=GEMINI_PROMPT)
    ...
    def process_ai_query(self):
        ...
        try:
            ...
            elif any(word in user_query_lower for word in FEAR_KEYWORDS): forced_emotion = "fear"
            elif any(word in user_query_lower for word in SURPRISE_KEYWORDS): forced_emotion = "surprise"
            
            self._log("🧠 Querying Gemini model...")
            
            response = self.gemini_model.generate_content(self.last_query)
            ...
            if forced_emotion:
                ...
                if last_part in ["happy", "sad", "angry", "pamper", "neutral", "fear", "surprise"]:
                    emotion = last_part
